...
struct Test::Main : Input_from_filter::Event_handler
{
	...
	Signal_handler<Test::Main> _config_handler { _env.ep(), *this, &Test::Main::_handle_config };
	...
	unsigned _num_steps = _config.xml().num_sub_nodes();
...
//			_env.parent().exit(0);
	...
	void _handle_config()
	{
		warning("config update");
		_config.update();
		_curr_step = 0;
		_num_steps = _config.xml().num_sub_nodes();
		log(_config.xml());
